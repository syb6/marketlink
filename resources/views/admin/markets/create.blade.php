...
<div class="row justify-content-center">
    <div class="col-lg-10">
        <div class="card-ml border-0 p-4 p-md-5">
            <h4 class="fw-bold mb-4">Market Details</h4>
            
            <form action="{{ route('admin.markets.store') }}" method="POST" enctype="multipart/form-data" class="form-ml">
                @csrf
                
                <div class="row g-4 mb-4">
                    <div class="col-md-8">
                        <label class="form-label fw-bold">Market Name *</label>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" value="{{ old('name') }}" required placeholder="e.g. Downtown Farmers Market">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" class="form-control @error('address') is-invalid @enderror" name="address" value="{{ old('address') }}" required placeholder="123 Main St">
                        ...
                        <div class="d-flex flex-wrap gap-3">
                            @foreach(['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'] as $day)
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="operating_days[]" value="{{ $day }}" id="day_{{ $day }}" {{ in_array($day, old('operating_days', [])) ? 'checked' : '' }}>
                    ...
                    <div class="col-md-6">
                        <div class="row g-3">
                            <div class="col-sm-6">
                                ...
                                <input type="time" class="form-control @error('opening_time') is-invalid @enderror" name="opening_time" value="{{ old('opening_time') }}">
                            </div>
                            <div class="col-sm-6">
                                <label class="form-label fw-bold">Closing Time</label>
                                <input type="time" class="form-control @error('closing_time') is-invalid @enderror" name="closing_time" value="{{ old('closing_time') }}">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-5">
                    <label class="form-label fw-bold">Market Image</label>
                    <input type="file" class="form-control @error('image') is-invalid @enderror" name="image" accept="image/*">
                    <div class="small text-muted mt-1">Recommended size: 1200x600px.</div>
                    @error('image')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                
                <div class="d-flex justify-content-end gap-2">
                    <a href="{{ route('admin.markets.index') }}" class="btn btn-outline-secondary px-4">Cancel</a>
                    <button type="submit" class="btn btn-primary-ml px-5">Create Market</button>
                </div>
            </form>
        </div>
    </div>
